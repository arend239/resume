...
#let resume(
  ...
  let contact-item(value, prefix: "", link-type: "", display: none) = {
    if value != "" {
      if link-type != "" {
        let content = if display != none { display } else { prefix + value }
        link(link-type + value)[#(content)]
      } else {
        value
      }
    }
    ...
    align(personal-info-position)[
      #{
        let items = (
          ...
          contact-item(github, display: "GitHub", link-type: "https://"),
          contact-item(linkedin, display: "LinkedIn", link-type: "https://"),
...
#let edu(
  ...
  if consistent {
    ...
    generic-two-by-two(
      top-left: strong(degree), // strong(institution),
      top-right: dates ,
      bottom-left: institution, // degree, //emph(degree),
      bottom-right: emph(location) //emph(dates),
    )
  }
}
